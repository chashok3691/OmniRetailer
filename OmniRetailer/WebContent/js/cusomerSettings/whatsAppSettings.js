...
$("#whatsAppGateway").keydown(function(){
	$("#whatsAppGatewayError").html("");
	$(".Error").html("");
}); 
...
function validateWhatsAppSettingsForm(operation){
	try{
		var whatsAppSettings = {},templatesList = [];
		if($("#whatsAppGateway").val().trim() == ""){
			$("#whatsAppGatewayError").html("Enter Gateway");
...
//				console.log(obj);
		...
		whatsAppSettings.templatesList=templatesList;
		whatsAppSettings.gateWay = $("#whatsAppGateway").val();
		whatsAppSettings.userName = $("#username").val();
		whatsAppSettings.password = $("#password").val();
		whatsAppSettings.enabled = enabled;
		
		if(parameterList.length > 0 )
			whatsAppSettings.parametersList = parameterList;

		whatsAppSettings.messageKey = $("#messageKey").val();
		whatsAppSettings.mobileKey = $("#mobileKey").val();
		whatsAppSettings.wattsappURL = $("#whatsAppURL").val();
		whatsAppSettings.mobileNumber=$("#mobilenumber").val();
		whatsAppSettings.templateId=$("#templateId").val();
		whatsAppSettings.textMsg=$("#textmessage").val();

		var formData = JSON.stringify(whatsAppSettings);
		console.log(formData);
		var contextPath = $("#contextPath").val();
		if(operation=="new"){
			URL = contextPath + "/settings/createWhatsAppSettings.do";
		}
		else if(operation=="edit"){

			URL = contextPath + "/settings/updateWhatsAppSettings.do";
		}
...
function validateWhatsAppMsgSetting(operation){
	debugger;
	try{
		...
		if($('#enabled').is(':checked'))
		{
	 	...
	 	finalObj.wattsappURL = $("#whatsAppURL").val();
	 	finalObj.gateWay = $("#whatsAppGateway").val();
		...
		if(operation=="new"){
			...
			URL = contextPath + "/settings/validateWhatsAppTestSettings.do";
		}
...
function addParameters()
{
	try{
		...
		$('#paramsTable tr:last').after('<tr   class="'+paramName.replace(/[^a-z0-9\s]/gi, '').replace(/[_\s]/g, '-')+paramValue.replace(/[^a-z0-9\s]/gi, '').replace(/[_\s]/g, '-')+'" ><td style="border: 1px solid lightgray;"><input type = "checkbox" class="CheckBoxChild" name="rowsSelected" value="'+paramName+"_"+paramValue+'""/></td><td style="border: 1px solid lightgray;">'+paramName+'</td><td style="border: 1px solid lightgray;">'+paramValue+'</td></tr>');
...
//		var whatsAppGateway = $("#whatsAppGateway").val(); 
		var whatsAppUrl = "";
		if( $("#whatsAppGateway").length>0)
			whatsAppUrl += $("#whatsAppGateway").val();
...
function validateWhatsAppUrl()
{
	...
	{
		whatsAppUrl  += "&"+$("#mobileKey").val()+"="+$("#mobileNo").val();
		

	}
	if( $("#messageKey").length>0)
	{
		whatsAppUrl += "&"+ $("#messageKey").val()+"=verified successfully" ;
...
function validateGateWay()
{
	...
	if( $("#whatsAppGateway").length>0)
	  ...
	  var expr = new RegExp("^http(s?)\:\/\/[0-9a-zA-Z]([-.\w]*[0-9a-zA-Z])*(:(0-9)*)*(\/?)([a-zA-Z0-9\-\.\?\,\'\/\\\+&amp;%\$#_]*)?$");
		...
		if(!expr.test(whatsAppUrl))
			{
				$("#whatsAppGatewayError").html("Invalid GateWay, start with http or https protocol.");
				$("#whatsAppGateway").focus();
				return;
			}	
