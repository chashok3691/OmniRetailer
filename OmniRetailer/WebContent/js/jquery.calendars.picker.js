...
$.extend(CalendarsPicker.prototype, {
	...
	_update: function(target, hidden) {
		...
		if (inst) {
			...
			if (inst.inline) {
				...
				//alert("generated content "+this._generateContent(target[0], inst));
	...
	_hidePlugin: function(target, immediate) {
		...
		if($("#selectedDate").length > 0)
			checkDateChanged();
			//showOutletDashboard('','','','');
		if($("#warrantFrom").length > 0)
			searchWarrant('','','0');
		if($("#warrantTo").length > 0)
			searchWarrant('','','0');
		if($("#feedbackFrom").length > 0 && $("#feedbackFrom").val() != "")
			searchCustomerFeedback('','','0');
		if($("#feedbackTo").length > 0)
			searchCustomerFeedback('','','0');
		...
		for (var week = 0; week < numWeeks; week++) {
			...
			for (var day = 0; day < calendar.daysInWeek(); day++) {
				...
				days += this._prepare(renderer.day, inst).replace(/\{day\}/g,
					...
					' class="dateSelect jd' + jd + ' ' + (dateInfo.dateClass || '') +
