...
<div class="row" >
  ...
  <div class="box box-primary" style="">
       <div class="box-header" style="text-align: center;padding-bottom: 0px; ">				
		<h3 class="box-title" style="border: 1px solid gray;padding: 5px;padding-left: 40px;padding-right: 40px;"><spring:message code="Purchases.and.payments.label"/></h3>
	    ...
	    <div class="box-body table-responsive"  >
             ...
             <div class="row">
                ...
                <div class="col-lg-3">
                  ...
                  <div class="row">
                   ...
                   <input type="button" style="margin-top:18px;width: 135%;" class="btn bg-olive btn-inline addBtn" onclick="getPurchasesAndPayments('','purchasesPayments','','false')" value="<spring:message code="search" />" />
                  </div>
                  <div class="row">
                  <a onclick="getPurchasesAndPayments('back','purchasesPayments','','false')" style="margin-top:31px;width: 135%;"  class="btn bg-olive btn-inline addBtn"><spring:message code="clearbutton.label"/></a>
                  </div>
                  </div></div>
                     <div class="row">
	           ...
	           <input type="button" style="float: right;" class="btn bg-olive btn-inline addBtn" onclick="newPurchasesAndPayments('${workflowsList[0].statusName}','');" value="<spring:message code="New.payment.label" />" />
	          </div>
           <div class="col-lg-1" style="float:right;padding-right:0px;margin-top:10px;padding-left:5px;">
            <input type="button" class="btn bg-olive btn-inline  addBtn" onclick="getPurchasesAndPayments('back','purchasesPayments','','true')" value="Save" >
            <input type="hidden" id="stockdownloadurl" value="${downloadurl}">                         
           </div>
           <div class="col-lg-1" style="float:right;padding-right:0px;margin-top:10px;padding-left:5px;">
            <input type="button" class="btn bg-olive btn-inline  addBtn" onclick="syncOrganizationMaster('','GRN')" value="Sync" >
           </div>
           </div>
		        </div>
                </div>
         ...
         <input type="hidden" id="fileDownloadurl" value="${filePath}">
         	<c:set var="totalReturninvoice" value="00"></c:set>
<div id="Error" class="Error" style="margin-top:-40px;text-align:center;color:red;font-weight:bold;">${err}</div>
  <div id="Success" class="Error" style="margin-top:-40px;text-align:center;color:blue;font-weight:bold;">${success}</div>
                  ...
                  <c:forEach var="paymentsList" items="${paymentsList}" varStatus="TheCount">
                   <tr>
                    <td>${TheCount.count+index-1}</td>
             	 	 ...
             	 	 <td>${paymentsList.supplierName}</td>
             	 	 <td>${paymentsList.invoiceAmount}</td>
             	 	 <td>${paymentsList.dueDateStr}</td>
             	 	 <td>${paymentsList.status}</td>    
             	 	 
             	 	 <c:if test="${paymentsList.transactionDescription=='Warehouse Shipment Return' || paymentsList.transactionDescription=='Shipment Return'}">
             	 	 <c:set var="totalReturninvoice" value="${paymentsList.invoiceAmount+totalReturninvoice}"></c:set>   
             	 	 </c:if>      	 
                      <td><a style="color:#2e7ea7 !important;cursor: pointer;margin-right:6px;" onclick="viewPurchasesAndPayments('','${paymentsList.transactionId}')">View</a>
                         <a style="color:#2e7ea7 !important;cursor: pointer;margin-right:6px;" onclick="editPurchasesAndPayments('','${paymentsList.transactionId}')">Open</a>       
                         <%--  <a style="color:#2e7ea7 !important;cursor: pointer;margin-right:6px;" onclick="getPurcphasePaymentsPDF('${paymentsList.transactionId}')">Print</a>  --%>       
                     </td>
                     </tr>
                </c:forEach> 
   ...
   <div class="col-sm-12 col-lg-12 col-xs-12">
        ...
        <div class="col-lg-6" style="float:right;margin-left: 100px;margin-right: auto;background-color:#EEEEEE;margin-top: 0px;border-style: solid;border-width: 1px;border-color:#EEEEEE;padding-top: 5px;font-size:14pt;padding-bottom: 10px;padding-left: 20px;color:solid black;" ">
          <fmt:formatNumber type="number" pattern="00" var="shipmentfinalOrdervalue" maxFractionDigits="2" minFractionDigits="0" value="${totalInvoice-totalReturninvoice}"/>
           <label style="width: 50%"><spring:message code="total.invoice.amount.label"/></label><label style="width: 50%;text-align: center;">${shipmentfinalOrdervalue}</label>
           </div>
        ...
        <div class="col-lg-6" style="float:right;margin-left: 100px;margin-right: auto;background-color:#EEEEEE;margin-top: 0px;margin-bottom: 50px;border-style: solid;border-width: 1px;border-color:#EEEEEE;padding-top: 5px;font-size:14pt;padding-bottom: 10px;padding-left: 20px;color:solid black;" ">
           <label style="width: 50%">Total Return Invoice Amount:</label><label style="width: 50%;text-align: center;">${totalReturninvoice}</label>
           </div>
        ...
        <input type="hidden" id="flowUnder" name="flowUnder" value= "${flowUnder}">
              ...
              <div class="row" style="padding-left: 15px;padding-right: 15px">
              	<c:if test="${index-1 == 0 && totalRecords >maxRecords}">
              		...
              		<input type="button" class='paginationButton' value="&#62;" onclick="getPurchasesAndPayments('data','purchasesPayments','${totalValue}','false')">
              	</div>
              	</c:if>
              	...
              	<c:if test="${totalValue == totalRecords}">
              			<c:if test="${totalValue-maxRecords >0}">
              				<div class="form-group col-lg-6" style="text-align: left;">
              		<input type="button" class='paginationButton' value="&#60;" onclick="getPurchasesAndPayments('data','purchasesPayments','${index - (maxRecords + 1)}','false');"> ${index} - ${totalValue} of ${totalRecords} 
              	...
              	<c:if test="${(totalRecords)>0}">
              		...
              		<div class="col-lg-2" style="padding-right: 0px;">
              		  <input type="button" onclick="getPurchasesAndPayments('data','purchasesPayments',document.getElementById('pagination').value,'false');" class="btn bg-olive btn-inline  addBtn" value="GO">
              		</div>
				</div>
