...
<div class="row">
              ...
              <div class="col-lg-9" style="padding-left:0px;padding-right: 0px;">
                 ...
                 <div class="col-lg-9">
                  <div class="row">
                   ...
                   <input type="button" style="margin-top:18px;width: 135%;" class="btn bg-olive btn-inline addBtn" onclick="creditNoteSummary('','creditNote','')" value="<spring:message code="search" />" />
                  </div>
                  <div class="row">
                  <a  type="button" style="margin-top:31px;width: 135%;"  class="btn bg-olive btn-inline addBtn" onclick="creditNoteSummary('clear','creditNote','')"><spring:message code="clearbutton.label"/></a>
              </div>
                  </div></div>
  ...
  <div id="Success" class="Error" style="text-align:center;color:blue;font-weight:bold;">${success}</div>
		...
		<div class="col-lg-2" style="padding-left:0px;padding-right:6px;">
	        <input type="button" style="float: right;" class="btn bg-olive btn-inline addBtn"   onclick="showCreditNote('new','creditNote','');" value="<spring:message code="credit.note.new.label" />" />
	     </div>
	     </div>                                     
         ...
         <table id="productstocks" class="table table-bordered table-striped" style="margin-top:0%;">
          <thead style="background-color: #3c8dbc; color: #ffffff">
                 <tr>  
                 	 ...
                 	 <th class="col-lg-3"><spring:message code="credit.note.ref.label" /></th>
                     ...
                     <th class="col-lg-2"><spring:message code="credit.note.value.label" /></th>
                 ...
                 <c:forEach var="DebitNote" items="${debitNoteSummary}" varStatus="TheCount">
             	 <tr>
					<td>${TheCount.count+index-1}</td>
             	 	 <td>${DebitNote.creditNoteRef}</td>
                 	 <td>${DebitNote.billDateStr}</td>
                 	 <td>${DebitNote.location}</td>
                     <td>${DebitNote.transactionDescription}</td>
                     <td>${DebitNote.creditNoteValue}</td>                    
                    <td><a style="color:#2e7ea7 !important;cursor: pointer;margin-right: 10px; " onclick="showCreditNote('view','creditNote','${DebitNote.creditNoteRef}')">View</a>  </td>                               	
                 </tr>
                </c:forEach>
            </tbody>
         </table>
...
<input type="hidden" id="flowUnder" name="flowUnder" value= "${flowUnder}">
              <div class="row" >
              	<c:if test="${index-1 == 0 && totalRecords >maxRecords}">
              		...
              		<input type="button" class='paginationButton' value="&#62;" onclick="creditNoteSummary('creditNote','','${totalValue}')">
              	</div>
              	</c:if>
              	...
              	<c:if test="${totalValue == totalRecords}">
              			<c:if test="${totalValue-maxRecords >0}">
              				<div class="form-group col-lg-6" style="text-align: left;">
              		<input type="button" class='paginationButton' value="&#60;" onclick="creditNoteSummary('creditNote','','${index - (maxRecords + 1)}');"> ${index} - ${totalValue} of ${totalRecords} 
              	...
              	<c:if test="${(totalRecords)>0}">
              		...
              		<div class="col-lg-2" style="padding-right: 0px;">
              		  <input type="button" onclick="creditNoteSummary('creditNote','',document.getElementById('pagination').value);" class="btn bg-olive btn-inline  addBtn" value="GO">
              		</div>
				</div>
