 ...
 * edited by 				: G Vasudeva Reddy
 * edit						:solved the problem of sku not being added to the table.
 * */ -->
...
if( $("#defaultstatus").val() == "Received"){
	$(".btnCls").removeClass('buttonMargin');	
}
                                ...
                                <form role="form">
                                    ...
                                    <div class="form-group col-lg-3">
                                        ...
                                        <div class="form-group col-lg-2">
                                            ...
                                            <input type="text" class="form-control" name="InspectedBy" id="inspected" placeholder='<spring:message code="enter.name.label"/>' value="${stockReceiptDetails.inspectedBy}"/>
                                   ...
                                   <div class="col-lg-12" style="background: #eee;padding: 10px 0px 10px 0px;">
     									<div class="col-lg-9">
      									...
      									<div class="col-lg-9" style="padding-left: 0px;    padding-top: 5px;">
      									  <c:forEach var="previousWorkFlowList" items="${previousList}">
                                     ...
                                     <div class="col-lg-12" style="background: #eee;padding: 10px 0px 10px 0px;">
                                        ...
                                        <div class="row">
                                         ...
                                         <input type="hidden" id="desc"/>
											...
											<div class="services">
								    			<div class="items">
								     				<ul class="matchedStringUl receipt" style="z-index: 3; "></ul>
								   				</div>
											</div>
  										 </div>
                                         <div class="col-lg-3">
...
<%--                   								<td id="Desc${theCount.count}" >${item.description}</td>
 --%>                  								<td id="UOM${theCount.count}" >${item.uom}</td>
                  								<td class='Price' id="Price${theCount.count}" >
                  								...
                  								<td id="Cost${theCount.count}" >${item.costPrice}</td>
                  								
                  								<td id="Supplied${theCount.count}" onBlur='changeSuppliedPack(this);' contenteditable='true'>${item.returnQty}</td>
                  								<td id="weighted${theCount.count}"  contenteditable='false'>${item.receivedQty }</td>
                  								<td class='Received' onBlur='changeReceivedPack(this);' id="Received${theCount.count}" contenteditable='true'>${item.acceptedQty}</td>
                  								<td class='Rejected' onBlur='changeRejected(this);' onkeypress='return (this.innerText.length < 10)' contenteditable='true' id="Rejected${theCount.count}" >${item.rejQty}</td>
                     ...
                     <table id="itemsList" class="table table-bordered table-hover">
                       ...
                       <tbody style="text-align: center;">
						...
						<div class="row" style="text-align: center;">
						  <c:if test="${stockReceiptDetails.status != 'Received'}">
						 <input type="button" class="btnCls" style="" onclick="validateStockReceipt('Approved','update','${stockReceiptDetails.goods_receipt_ref_num}','${stockReceiptDetails.id_goods_receipt}');" value="Submit" />
						 </c:if>
						<%-- <input type="button" class="btnCls" style="margin-left:15px;" onclick="validateStockReceipt('Draft','update','${stockReceiptDetails.goods_receipt_ref_num}','${stockReceiptDetails.id_goods_receipt}');" value="<spring:message code="save.label" />" /> --%>
						...
						<c:if test="${sessionScope.role != 'super admin' && flowUnder == 'outlet'}">
		           		<c:forEach var="accessControl" items="${sessionScope.outletGoodsTransfer}">
		                	<c:if test="${accessControl.appSubDocument == 'OutletStockReceipt' && accessControl.write == true && accessControl.read == true}">
		                		<input type="button" class="btnCls buttonMargin" style="" onclick="searchStockReceipts('','all','0');" value="<spring:message code="cancel.label" />" />
		                	</c:if>
		                </c:forEach>
		             </c:if>
					<c:if test="${sessionScope.role == 'super admin' && flowUnder == 'outlet'}">
