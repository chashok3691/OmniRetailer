...
$('#supplierReport').on('input',function(e){
	if($(this).val().trim()==""){
		viewCustomerReferrals('CustomerReferal','0','false')
	}
});
...
<div class="row">
  ...
  <div class="box box-primary">
          <div class="box-header" align="center">
				<h3 class="box-title" >
					<spring:message code="customer.refral.label" />
				</h3>			
				</div><!-- /.box-header -->
                  ...
                  <div class="row">
                     ...
                     <div class="col-lg1-2 col-lg-3 form-group">
                       <%-- <div class="col-lg-12" style="padding-left:0px;padding-right: 0px;">
                           ...
                           <select id="customerType"  class="form-control clearError" >
...
<%-- 							          <option value="${location}" ${location == storeLocation ? 'selected' : ''} id="${location}" >${location}</option> --%>
	               ...
	               <div class="col-lg-8" style="padding-left:0px;padding-right: 0px;">
	                   <label><spring:message code="offer_end_date.label"/></label>
	                 	<input class="form-control calendar_icon form-group" readonly="readonly" value="" style="background-color: white;" id="toDate" size="20" type="text" onfocus="callCalender('toDate')" onclick="callCalender('toDate')" placeholder="DD/MM/YYYY" />
		           </div>
		          </div>
                    </div>
                  </div>
                  ...
                  <div class="row">
                  <a onclick="viewCustomerReferrals('CustomerReferal','0','false');" class="btn bg-olive btn-inline" style="padding: 4px 8px;margin-bottom: 16px;width: 100%;margin-top: 16px;"><spring:message code="searchbutton.label"/></a>
                  </div>
                  <div class="row">
		 ...
		 <div class="col-lg-1 col-xs-2" style="padding-left: 0px;">
                <select class="form-control" id="maxRecords" style="padding: 3px 4px;" onchange="viewCustomerReferrals('CustomerReferal','0','false');">
                  ...
                  <div class="col-lg-9 col-xs-12" style="padding-left: 0px;padding-right: 0px;">
                     ...
                     <div class="col-lg-1" style="text-align: center;padding-right: 0px;padding-bottom: 4px;">
	        		  ...
	        		  <input type="button" class="addBtn btn bg-olive btn-inline" onclick="return vewNewCustomerReferrals();" style="" value="<spring:message code="addnew.label" />">
				</div>	
               
                    
                    <div class="col-lg-1" style="text-align: center;padding-right: 0px;padding-bottom: 4px;">
		   <input type="button" class="addBtn btn bg-olive btn-inline" data-toggle="modal" data-target="#myModalIos"  style="" value="<spring:message code="save.label" />" />
		 </div>
              </div>      
		     ...
		     <table id="example1" class="table table-bordered table-striped">
               ...
               <tr>
                  <th><div ><spring:message code="s.no.label" /></div></th>
                   <th><div ><spring:message code="referral.programId.label" /></div></th>
                   <th><div><spring:message code="program.description.label" /></div></th>
                   <th><spring:message code="import.Start_Date.label" /></th>
                   <th><spring:message code="import.End_Date.label" /></th>
                    <th><spring:message code="program.value.label" /></th>
                   <th><spring:message code="occasion.status.label" /></th>
                   <th><spring:message code="action.label"/></th>
               </tr>
           </thead>
           <tbody style="text-align: center;">
            <c:forEach var="customerDetails" items="${customerReferralList}" varStatus="theCount">
          <tr>
           <td>${theCount.count}</td>
           <td>${customerDetails.referralProgramId}</td>
           <td>${customerDetails.programDescription}</td>
           <td>${customerDetails.startDateStr}</td>
           <td>${customerDetails.endDateStr}</td>
           <td>${customerDetails.minumumPurchaseAmount}</td>
           <td>${customerDetails.status}</td>
           <td>
              <a style="color:#2e7ea7 !important;cursor: pointer;" onclick="return viewEditCustomerReferral('${customerDetails.referralProgramId}','view')"><spring:message code="procurement.view.label" /></a>&nbsp;&nbsp;
              <a style="color:#2e7ea7 !important;cursor: pointer;" onclick="return viewEditCustomerReferral('${customerDetails.referralProgramId}','edit')"><spring:message code="edit.label" /></a>&nbsp;&nbsp;
           </td>
           </tr>
           </c:forEach>
           </tbody>
       ...
       <div class="row">
        <input type="hidden" id="flowUnder" value="${flowUnder}">
              	<c:if test="${index-1 == 0 && totalRecords >maxRecords}">
              		...
              		<input type="button" class='paginationButton' value="&#62;" onclick="viewCustomerReferrals('CustomerReferal','${totalValue}','false')">
              	</div>
              	</c:if>
              	...
              	<c:if test="${totalValue == totalRecords}">
              			<c:if test="${totalValue-maxRecords >0}">
              				<div class="form-group col-lg-6" style="text-align: left;">
              		<input type="button" class='paginationButton' value="&#60;" onclick="return viewCustomerReferrals('CustomerReferal','${index - (maxRecords + 1)}','false');"> ${index} - ${totalValue} of ${totalRecords} 
              	...
              	<c:if test="${(totalRecords)>0}">
              		...
              		<div class="col-lg-2" style="padding-right: 0px;">
              		  <input type="button" onclick="viewCustomerReferrals('CustomerReferal',document.getElementById('pagination').value,'false')" class="btn bg-olive btn-inline  addBtn" value="GO">
              		</div>
              	</div>
...
<div class="col-lg-6 col-sm-6 col-xs-6" style="padding-left: 5px; padding-right: 5px;">
 <input type="button" class="iosclosebutton1" onclick="viewCustomerReferrals('CustomerReferal','0','true')"  value="Ok">
</div>
<div class="col-lg-6 col-sm-6 col-xs-6" style="padding-left: 5px; padding-right: 5px;">
