  ...
  <div class="box box-primary">
	 ...
	 <c:if test="${status == 'Delivered'}"> 
      ...
      <c:if test="${status == 'Rejected'}"> 
          <h3 class="box-title"><spring:message code="returned.shipments.label" /></h3>
     </c:if>  --%>
        ...
        <div class="row">
	        <div class="col-lg-3">
               <div class="row">
                 ...
                 <div class="col-lg-8 col-xs-8 col-sm-10 col-md-10" style="">
	             <label><spring:message code="location.label"/></label>
	             <select class="form-control form-group" id="outletLocation" onchange="searchShipments('${status}','','0');">
<%--                       <option value=""><spring:message code="ALL.label"/></option> --%>
	        ...
	        <div class="col-lg-2">
              <div class="row">
                	<div class="col-lg-4 col-xs-4 col-sm-2 col-md-2" style="text-align:left;top:5px;padding-right: 0px;">
                	  ...
                	  <div class="col-lg-10 col-xs-10 col-sm-10 col-md-10" style="padding-left: 0px;">
                	    <label><spring:message code="offer_start_date.label"/></label>
            ...
            <div class="col-lg-2">
               <div class="row">
                <%--  <div class="col-lg-4 col-xs-4 col-sm-2 col-md-2" style="text-align:left;top:5px;padding-right: 0px;">
                 ...
                 <div class="col-lg-10 col-xs-10 col-sm-10 col-md-10" style="">
                   <label><spring:message code="offer_end_date.label"/></label>
           ...
           <div class="col-lg-1" style="margin-top: 18px;padding-left: 0px;">
                    <input type="hidden" id="type" value="${type}">
                        <input type="button" style="" class="btn bg-olive btn-inline  addBtn" onclick="searchShipments('','','0')" value="<spring:message code="search" />" />
           </div>
	      </div>
	      ...
	      <div class="col-lg-1 col-xs-2" style="margin-top: 10px;">
	        ...
	        <div class="col-lg-1" style="margin-top: 10px;padding-left: 0px;">
				...
				<c:if test="${sessionScope.role == 'super admin'}">
	        		<input type="button" onclick="return viewNewShipment();" class="btn bg-olive btn-inline  addBtn" value="<spring:message code="addnew.label" />">
	      		</c:if>
				<c:if test="${sessionScope.role != 'super admin'}">
					<c:if test="${flowUnder == 'outlet'}">
			           <c:forEach var="accessControl" items="${sessionScope.outletShipment}">
			                <c:if test="${accessControl.appDocument == 'outletShipment' && accessControl.write == true}"> 
			                	<input type="button" onclick="return viewNewShipment();" class="btn bg-olive btn-inline  addBtn" value="<spring:message code="addnew.label" />">
							</c:if>
						</c:forEach>
					</c:if>
					...
					<c:if test="${flowUnder == 'management'}">
			           <c:forEach var="accessControl" items="${sessionScope.orderManagemetSalesOrderFulfilment}">
			                <c:if test="${accessControl.appDocument == 'OrderManagementFulFillment' && accessControl.write == true}"> 
			                	<input type="button"  onclick="return viewNewShipment();" class="btn bg-olive btn-inline  addBtn" value="<spring:message code="addnew.label" />">
							</c:if>
						</c:forEach>
					</c:if>
            ...
            <tbody style="text-align: center;">
            	<c:forEach var="shipper" items="${shipmentBeanList}">
	              <tr>
                    ...
                    <td>${shipper.contactNum}</td>
                    <td>${shipper.shipmentMode}</td>
                    <td>
                   		<a style="color:#2e7ea7 !important;cursor: pointer;" onclick="return viewShipmentDetails('${shipper.shipmentId}','view')">View</a>
                   		<c:if test="${sessionScope.role == 'super admin'}">
						...
						<c:if test="${sessionScope.role != 'super admin'}">
							<c:if test="${flowUnder == 'outlet'}">
					           <c:forEach var="accessControl" items="${sessionScope.outletShipment}">
					                <c:if test="${accessControl.appDocument == 'outletShipment' && accessControl.write == true}"> 
					                	&nbsp;&nbsp;<a style="color:#2e7ea7 !important;cursor: pointer;" onclick="return viewShipmentDetails('${shipper.shipmentId}','edit')">Edit</a>
									</c:if>
								</c:forEach>
							</c:if>
							<c:if test="${flowUnder == 'warehouse'}">
