...
callCalender('fromGRNReceipt');
callCalender('toGRNReceipt');
...
$('#supplierReport').on('input',function(e){
	if($(this).val().trim()==""){
		viewOMShipments('outletShipmentsReport','0','','false')
	}
});
...
$('#mySearch').click(function(){
    ...
    viewOMShipments('outletShipmentsReport','0',search,'false');
})
...
<div class="row">
  ...
  <div class="box box-primary">
          <div class="box-header" align="center">
				<h3 class="box-title" >
					Item Wise-<spring:message code="shipment.receipt.label" />
				</h3>			
				</div><!-- /.box-header -->
              ...
              <div class="col-lg-8" style="padding-left:0px;padding-right: 0px;">
                	 	...
                	 	<input class="form-control calendar_icon form-group" readonly="readonly" value="${startdate}" style="background-color: white;" id="fromGRNReceipt" size="20" type="text" onfocus="callCalender('fromGRNReceipt')" onclick="callCalender('fromGRNReceipt')" placeholder="DD/MM/YYYY" />
	              </div>
	            </div>
...
<%-- 									<option value="${location}" ${location == selectedLocation ? 'selected' : ''} id="${location}" >${location}</option> --%>
	               ...
	               <div class="col-lg-8" style="padding-left:0px;padding-right: 0px;">
	                   <label><spring:message code="offer_end_date.label"/></label>
	                 	<input class="form-control calendar_icon form-group" readonly="readonly" value="${enddate}" style="background-color: white;" id="toGRNReceipt" size="20" type="text" onfocus="callCalender('toGRNReceipt')" onclick="callCalender('toGRNReceipt')" placeholder="DD/MM/YYYY" />
		           </div>
		          </div>
                    </div>
                  </div>
                  ...
                  <div class="row">
                  <a onclick="viewOMShipments('outletShipmentsReport','${index-1}','${searchName}','false')" class="btn bg-olive btn-inline" style="padding: 4px 8px;margin-bottom: 16px;width: 100%;margin-top: 16px;"><spring:message code="searchbutton.label"/></a>
                  </div>
                  <div class="row">
		 ...
		 <div class="col-lg-1 col-xs-2" style="padding-left: 0px;">
                <select class="form-control" id="maxRecords" style="padding: 3px 4px;" onchange="viewOMShipments('outletShipmentsReport','${index-1}','${searchName}','false')">
                  ...
                  <div class="col-lg-10 col-xs-12" style="padding-left: 0px;padding-right: 0px;">
                    ...
                    <div class="col-lg-1" style="text-align: center;padding-right: 0px;padding-bottom: 4px;margin-top: -22px;">
		   <input type="button" class="addBtn btn bg-olive btn-inline" data-toggle="modal" data-target="#myModalIos"  style="font-size: 16px;" value="<spring:message code="save.label" />" />
		 </div>
...
<!--                     <div id="example1_wrapper" style="" class="dataTables_wrapper form-inline" role="grid"> -->
				         <table id="example1" class="table table-bordered table-striped">
                            <thead style=" color: #ffffff">
                               <tr>
                                	<th style="border: 2px solid #444 !important;"><spring:message code="report.sno.label" /></th>
                                	<th  style="border: 2px solid #444 !important;">GRN No</th>
                                	<th  style="border: 2px solid #444 !important;">Invoice No</th>
                                	<th  style="border: 2px solid #444 !important;">GRN Date</th>
                                	 <th style="border: 2px solid #444 !important;"><spring:message code="location.label" /></th>
                                	<th style="border: 2px solid #444 !important;">Supplier Code</th>
...
<%--                                 	<th style="border: 2px solid #444 !important;"><spring:message code="date.label" /></th> --%>
                                   ...
                                   <th style="border: 2px solid #444 !important;">Category</th>
                                    <th style="border: 2px solid #444 !important;"><spring:message code="ean.label" /></th>
                                    ...
                                    <th style="border: 2px solid #444 !important;"><spring:message code="price.label" /></th>
                                    <th style="border: 2px solid #444 !important;"><spring:message code="tax_amount.label" /></th>
                                   <th style="border: 2px solid #444 !important;">Shipment Charge</th>
                                    <th style="border: 2px solid #444 !important;"><spring:message code="report.proccost.label" /></th>
                                   
                            </thead>
                           <tbody style="text-align: center;">
                            <c:set var="procQty" value="0"  />
                             <c:set var="procurementPrice" value="0"  />
                              <c:set var="procTax" value="0"  />
                               <c:set var="procurementCost" value="0"  />
                           
                             <c:forEach var="warehousereport" items="${warehousereport}" varStatus="thecount">
                          	 <tr>
                                 <td>${thecount.index+index}
                                 </td>
                                 <td>${warehousereport.grnNo}</td>
                                  <td>${warehousereport.invoiceRefNo}</td>
                                   <td>${warehousereport.grnDate}</td>
                                 <td>${warehousereport.location}</td>
                                 <td>${warehousereport.supplierId}</td>
                                 <td>${warehousereport.supplierName}</td>
<%--                                  <td>${warehousereport.dateStr} --%>
<!--                                  </td> -->
                                 <td class="graph" id="skuid${thecount.index+index}">${warehousereport.skuID}</td> 
                                  <td style="text-align: left;">${warehousereport.itemDescription}</td> 
                                  <td>${warehousereport.category}</td>
                                    <td id="ean${thecount.index+index}">${warehousereport.ean}</td>
                                     <td>${warehousereport.uom}</td> 
                                      <td id="Pqty${thecount.index+index}">${warehousereport.procQty}</td> 
                                      <td id="Pqprice${thecount.index+index}">${warehousereport.procPrice}</td>
                                      <td>${warehousereport.taxValue}</td>
                                      <td>${warehousereport.shipmentCharges}</td>
                                      <td id="Cqty${thecount.index+index}">${warehousereport.procCost}</td> 
                                       	 <c:set var="procQty" value="${procQty+((warehousereport.procQty))}" scope="page" />
                                       	 <c:set var="procTax" value="${procTax+((warehousereport.taxValue))}" scope="page" />
                                         <c:set var="procurementPrice" value="${procurementPrice+((warehousereport.procPrice))}" scope="page" />
                                          <c:set var="procurementCost" value="${procurementCost+((warehousereport.procCost))}" scope="page" /> 
                              </tr>
              ...
              <div class="row">
               <input type="hidden" id="flowUnder" value="${flowUnder}">
              	<c:if test="${index-1 == 0 && totalRecords >10}">
              		...
              		<input type="button" class='paginationButton' value="&#62;" onclick="viewOMShipments('outletShipmentsReport','${totalValue}','${searchName}','false')">
              	</div>
              	</c:if>
              	...
              	<c:if test="${totalValue == totalRecords}">
              			<c:if test="${totalValue-10 >0}">
              				<div class="form-group col-lg-6" style="text-align: left;">
              		<input type="button" class='paginationButton' value="&#60;" onclick="return viewOMShipments('outletShipmentsReport','${index - 11}','${searchName}','false');"> ${index} - ${totalValue} of ${totalRecords} 
              	...
              	<c:if test="${(totalRecords)>0}">
              		...
              		<div class="col-lg-2" style="padding-right: 0px;">
              		  <input type="button" onclick="viewOMShipments('outletShipmentsReport',document.getElementById('pagination').value,'${searchName}','false')" class="btn bg-olive btn-inline  addBtn" value="GO">
              		</div>
              		</div>
...
<div class="col-lg-6 col-sm-6 col-xs-6" style="padding-left: 5px; padding-right: 5px;">
 <input type="button" class="iosclosebutton1" onclick="viewOMShipments('outletShipmentsReport','0','','true')"  value="Ok">
</div>
<div class="col-lg-6 col-sm-6 col-xs-6" style="padding-left: 5px; padding-right: 5px;">
