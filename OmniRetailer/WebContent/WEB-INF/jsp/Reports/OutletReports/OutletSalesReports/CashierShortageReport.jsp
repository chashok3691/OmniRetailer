...
<!-- Main content -->
				...
				<section class="content-header">
                    <h1>
                      ...
                      </c:if>&nbsp;<spring:message code="sales.reports.label"/>&nbsp;&#45;&nbsp;<spring:message code="cashier.shortage.report.label"/>
                    </h1>
...
<%-- 									<option value="${location}" ${location == selectedLocation ? 'selected' : ''} id="${location}" >${location}</option> --%>
                       ...
                       <div class=" col-lg-2">
                       <div class="col-lg-12" style="padding-left:0px;padding-right: 0px;">
                           <label><spring:message code="cashier.label"/></label>
                          <select class="form-control" id="cashierName" >
                           		 <option value=""><spring:message code="ALL.label"/></option> 
	                            <c:forEach var="cashLocation" items="${employeeList}">
									<option value="${cashLocation}">${cashLocation}</option>
						 		</c:forEach>
	                        </select>
                      </div>
                      </div>
                  ...
                  <div class="row">
                 <a onclick="viewCashierShortageReport('cashierShortageReport','0','','','${flowLocation}')" class="button"><spring:message code="searchbutton.label"/></a>
                  </div>
                  <div class="row">
                  <a onclick="viewCashierShortageReport('cashierShortageReport','0','','clear','${flowLocation}')" class="button"><spring:message code="clearbutton.label"/></a>
                  </div>
                  </div></div> 
                  <div class="col-lg-12">
                  <br>
 ...
 <br><br>
		   ...
		   <input type="button" class="addBtn btn bg-olive btn-inline" onclick="viewCashierShortageReport('cashierShortageReport','0','save','','${flowLocation}')"  style="font-size: 16px;" value="<spring:message code="save.label" />" />
		 </div> 
				                  </div>
                                    ...
                                    <table id="productstocks" class="table table-bordered table-striped">
                                     <thead style="background-color: #3c8dbc; color: #ffffff">
                                        ...
                                        <tbody style="text-align: center;">
                                            <c:forEach var="items" items="${warehousereport.reportList}" varStatus="theCount">
                                        	 <tr>
                                        	 	<td>${theCount.index+index}</td>
                                        	    <td>${items.date}</td>
                                        	 	<td>${items.storeLocation}</td>
                                        	 	<td>${items.counterId}</td>
                                      
                                        	 	<td>${items.cashierId}</td>
                                        	 	<td>${items.shiftId}</td>
                                        	 	<td></td>
                                        	 	<td>${items.systemAmount}</td>
                                        	 	<td>${items.declareAmount}</td>
                                        	 	<td>${items.shortageAmount}</td>
              ...
              <div class="row">
               ...
               <input type="hidden" id="totalnum" value="${totalRecords}">
              	<c:if test="${index-1 == 0 && totalRecords >10}">
              		...
              		<input type="button" class='paginationButton' value="&#62;" onclick="viewCashierShortageReport('cashierShortageReport','${totalValue}','','','${flowLocation}')">
              	</div>
              	</c:if>
              	...
              	<c:if test="${totalValue == totalRecords}">
              			<c:if test="${totalValue-10 >0}">
              				<div class="form-group col-lg-6" style="text-align: left;">
              		<input type="button" class='paginationButton' value="&#60;" onclick="return viewCashierShortageReport('cashierShortageReport','${index - 11}','','','${flowLocation}');"> ${index} - ${totalValue} of ${totalRecords} 
              	...
              	<c:if test="${(totalRecords)>0}">
              		...
              		<div class="col-lg-2" style="padding-right: 0px;">
              		  <input type="button" onclick="viewCashierShortageReport('cashierShortageReport',document.getElementById('pagination').value,'','','${flowLocation}')" class="btn bg-olive btn-inline  addBtn" value="GO">
              		</div>
