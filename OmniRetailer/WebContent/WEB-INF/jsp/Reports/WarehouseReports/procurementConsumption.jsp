...
$('#consumption').on('input',function(e){
	if($(this).val().trim()==""){
		viewProcurementConsumptionreport('warehouseProcurementVsConsumption','0', '');
	}
});


$('#mySearch').click(function(){
    var search = $('#consumption').val();
    return viewProcurementConsumptionreport('warehouseProcurementVsConsumption','0', search);
})
...
<div class="row">
  ...
  <div class="box box-primary">
          <div class="box-header" align="center">
				<h3 class="box-title" >
					<spring:message code="procurementvsconsumption.label" />
				</h3>			
				</div><!-- /.box-header -->
                        ...
                        <label><spring:message code="zone.label"/></label>
                           <select class="form-control" id="outletZone" onchange="searchlocationsBasedOnZoneForDashboard('outletZone','outletLocation','warehouse')">
	                            ...
	                            <c:forEach var="zoneList" items="${zoneList}">
									<option value="${zoneList.zoneID}" ${zoneList.zoneID == zone ? 'selected' : ''}  >${zoneList.zoneID}</option>
						 		</c:forEach>
	                        </select>
                         </div>
                      </div>
              ...
              <div class="col-lg-8" style="padding-left:0px;padding-right: 0px;">
                  ...
                  <div class="row">
                  <a onclick="viewProcurementConsumptionreport('warehouseProcurementVsConsumption','0','')" class="button"><spring:message code="searchbutton.label"/></a>
                  </div>
                  <div class="row">
                  ...
                  <div class="col-lg-11 col-xs-11" style="padding-bottom:3px;padding-left: 0px;padding-right: 0px;">
                      	
                      	<div class="wrapper">
	        	 		 <input type="text"  class="form-control" value="${searchName}" id="consumption" placeholder="<spring:message code="search.items.here.label" />" />
						<!-- 	<input type="button" value="Search" id="mySearch"> -->
						<button id="mySearch" class="searchbutton"></button>
						 
							</div>
                      	
                      	 <%-- <input type="text" value="${searchName}"   class="form-control searchItems searchBar" style=""  id="consumption" placeholder="<spring:message code="search.items.here.label" />" />
							<div class="services">
				    			<div class="items">
				     				<ul class="matchedStringUl consumption" style=""></ul>
				   				</div>
							</div> --%>
					<%-- 	<input type="hidden" id="stock" value="${searchName}"> --%>
                    </div>
                             <div class="col-lg-1" style="">
                     <input type="button"  class="btn bg-olive btn-inline  addBtn" onclick="viewProcurementConsumptionreport('warehouseProcurementVsConsumption','0','true')" value="SAVE"/>
                               ...
                               <tr>
                                    ...
                                    <th style="border: 2px solid #444 !important;"><spring:message code="report.uom.label" /></th>
                                       ...
                                       <th style="border: 2px solid #444 !important;"><spring:message code="report.procqty.label" /></th>
                                          <th style=" border: 2px solid #444 !important;"><spring:message code="report.consqty.label" /></th>
                                             <th style="border: 2px solid #444 !important;"><spring:message code="report.proccost.label" /></th>
                                        <th style="border: 2px solid #444 !important;"><spring:message code="report.conscost.label" /></th>
                                </tr>
                            </thead>
                                  ...
                                  <td>${warehousereport.itemDescription}</td> 
                                   <td>${warehousereport.uom}</td> 
                                      <td id="Pqty${thecount.index+index}">${warehousereport.procQty}</td> 
                                       <td id="Cqty${thecount.index+index}">${warehousereport.conQty}</td> 
                                        <td>${warehousereport.procCost}</td> 
                                         <td>${warehousereport.conCost}</td> 
              ...
              <div class="row">
               <input type="hidden" id="flowUnder" value="${flowUnder}">
              	<c:if test="${index-1 == 0 && totalRecords >10}">
              		...
              		<input type="button" class='paginationButton' value="&#62;" onclick="viewProcurementConsumptionreport('warehouseProcurementVsConsumption','${totalValue}','${searchName}')">
              	</div>
              	</c:if>
              	...
              	<c:if test="${totalValue == totalRecords}">
              			<c:if test="${totalValue-10 >0}">
              				<div class="form-group col-lg-6" style="text-align: left;">
              		<input type="button" class='paginationButton' value="&#60;" onclick="return viewProcurementConsumptionreport('warehouseProcurementVsConsumption','${index - 11}','${searchName}');"> ${index} - ${totalValue} of ${totalRecords} 
              	...
              	<c:if test="${(totalRecords)>0}">
              		...
              		<div class="col-lg-2" style="padding-right: 0px;">
              		  <input type="button" onclick="viewProcurementConsumptionreport('warehouseProcurementVsConsumption',document.getElementById('pagination').value,'${searchName}')" class="btn bg-olive btn-inline  addBtn" value="GO">
              		</div>
              		</div>
