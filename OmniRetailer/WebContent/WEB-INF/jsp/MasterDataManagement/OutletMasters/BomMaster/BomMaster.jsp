 ...
 $(document).ready(function(){
	/*  $("#searchPriceRange").val($("#priceRangePrice").val());
	 ...
	 $('#mySearch').click(function(){
	      ...
	      return viewBomMaster('BomMaster','0');
	  })
   ...
   <br>
      ...
      <div class="row">
      	  <div class="col-lg-1 col-xs-1">
				                      	<select class="form-control" id="maxRecords" style="padding: 3px 4px;" onchange="viewBomMaster('BomMaster','0')">
							...
							<input type="hidden" id="BOMPrice" value="${searchName}">
                               ...
                               <c:if test="${sessionScope.role == 'super admin'}"> 
                                	<div class="col-lg-1" style="padding-right: 0px;padding-bottom: 5px;">
		  				               <input type="button" onclick="return newBomMaster('BomMaster');" class="btn bg-olive btn-inline addBtn" value="<spring:message code="addnew.label" />">
                               ...
                               <%--   <input type="hidden" value="${rates.bomRef}"> --%>
                                  ...
                                  <c:forEach var="accessControl" items="${sessionScope.outletMasterSettings}">
									<c:if test="${accessControl.appDocument == 'BOMMaster' && accessControl.write == true}"> 
                                  	  ...
                                  	  <td><a style="color:#2e7ea7 !important;cursor: pointer;" onclick="return viewEditBomMaster('${rates.bomRef}','view')">View</a>&nbsp;&nbsp;&nbsp;
                                       <c:if test="${sessionScope.role == 'super admin'}"> 
                                       	<a style="color:#2e7ea7 !important;cursor: pointer;" onclick="return viewEditBomMaster('${rates.bomRef}','edit')">Edit</a>
                                       </c:if>
                                      <c:forEach var="accessControl" items="${sessionScope.outletMasterSettings}">
										<c:if test="${accessControl.appDocument == 'BOMMaster' && accessControl.write == true}"> 
                                      		<a style="color:#2e7ea7 !important;cursor: pointer;" onclick="return viewEditBomMaster('${rates.bomRef}','edit')">Edit</a>
              	...
              	<c:if test="${index-1 == 0 && totalRecords > maxRecords}">
              		...
              		<input type="button" class='paginationButton' value="&#62;" onclick="viewBomMaster('BomMaster','${index+maxRecords-1}')">
              	</div>
              	</c:if>
              	...
              	<c:if test="${totalValue == totalRecords}">
              			<c:if test="${totalValue-maxRecords >0}">
              				<div class="form-group col-lg-2">
              		<input type="button" class='paginationButton' value="&#60;" onclick="return viewBomMaster('BomMaster','${index - maxRecords - 1}');"> ${index} - ${totalValue} of ${totalRecords} 
              		<input type="button" class='paginationButton' value="&#62;" >
              		</div>
              			</c:if>
              			<c:if test="${totalValue-maxRecords <= 0}">
              	...
              	<c:if test="${index-1 != 0 &&  totalValue != totalRecords}">
              		<div class="form-group col-lg-2">
              		<input type="button" class='paginationButton' value="&#60;" onclick="return viewBomMaster('BomMaster','${index - maxRecords-1}');"> ${index} - ${index+maxRecords-1} of ${totalRecords} .
              		<input type="button" class='paginationButton' value="&#62;" onclick="viewBomMaster('BomMaster','${index+maxRecords-1}')">
              	</div> 
              	</c:if>
              	...
              	<c:if test="${(totalRecords)>0}">
              		...
              		<div class="col-lg-2" style="padding-right: 0px;">
              		  <input type="button" onclick="viewBomMaster('BomMaster',document.getElementById('pagination').value)" class="btn bg-olive btn-inline  addBtn" value="GO">
              		</div>
				</div>
              </div>   
