...
$('#mySearch').click(function(){
    ...
    viewServiceAreaMaster('serviceAreaMaster','0');
})
...
<!--                            		<option value="">ALL</option> -->
                           		<option value="true"${'true' == status ? 'selected' : ''}>Active</option>
                           		<option value="false"${'false' == status ? 'selected' : ''}>Inactive</option>
                           </select>
                      </div>
                      </div>
                 ...
                 <div class="col-lg-1" style="    width: 12%;    padding: 10px 15px;    background: #ccc;">
                  ...
                  <div class="row">
                  <a onclick="viewServiceAreaMaster('serviceAreaMaster','0');" style="padding: 4px 8px;     margin-bottom: 16px;   width: 100%;margin-top: 16px;" class="btn bg-olive btn-inline"><spring:message code="searchbutton.label"/></a>
                  </div>
                  <div class="row">
                        ...
                        <div id="Success" class="Error" style="text-align: center;color: blue;font-size: 2;font-weight: bold;margin-bottom: 10px; margin-top: -20px">${success}</div>
                          ...
                          <div class="row">
                            <div class="col-lg-1 col-xs-1">
				                      	<select class="form-control" id="maxRecords" style="padding: 3px 4px;" onchange="viewServiceAreaMaster('serviceAreaMaster','0')">
    ...
    margin-right: 2%;">
	        	 		 <%-- <input type="text"  class="form-control searchItems searchBar"  value="" id="searchZone" placeholder="<spring:message code="search.zone.label" />" />
							...
							<input type="text"  class="form-control" value="${searchName}" id="searchRoute" placeholder="<spring:message code="searchServiceAreaMasters.label" />" />
                     ...
                     <c:forEach var="accessControl" items="${sessionScope.genericBusinessSettings}">
                          ...
                          <div class="col-lg-1" style="padding-left: 0px;">
								                <input type="button" style="" class="btn bg-olive btn-inline addBtn" onclick="return newServiceAreaMaster('serviceAreaMaster');" value="<spring:message code="addnew.label" />">
								           </div>
								           <div class="col-lg-1" style="padding-left: 0px;">
								                <input type="button" style="" class="btn bg-olive btn-inline addBtn" onclick="return validate('serviceAreaMaster');" value="<spring:message code="delete.label" />">
								           </div>
						 </c:if>
					</c:forEach>
					</div>
...
<%--                                   <th><spring:message code="newarrival.enddate" /></th> --%>
                         ...
                         <c:forEach var="serviceAreaList" items="${serviceAreaList}"  varStatus="theCount">
                          <tr>
                         <c:if test="${sessionScope.role == 'super admin'}"> 
                              	<td><input type="checkbox" name="selectall" class="selectableCheckbox" id="selectall" value='${serviceAreaList.area_code}' /></td>
                              </c:if>
                            <c:forEach var="accessControl" items="${sessionScope.genericBusinessSettings}">
                          		<c:if test="${accessControl.appDocument == 'ServiceAreaMaster' && accessControl.write == true}"> 
                             		<td><input type="checkbox" name="selectall" class="selectableCheckbox" id="selectall" value='${serviceAreaList.area_code}' /></td>
                            ...
                            <td>${serviceAreaList.area_code}</td>
                            <td>${serviceAreaList.city}</td>
                            
                            <c:choose>
                            <c:when test="${serviceAreaList.status == 'true'}">
                            <td>Active</td>
                            </c:when>
                           <c:otherwise>
                           <td>Inactive</td>
                             ...
                             <td>${serviceAreaList.priority}</td>
                             <td>${serviceAreaList.createDateStr}</td>
<%--                              <td>${routeList.endDateStr}</td> --%>
                              <td><a style="color:#2e7ea7 !important;cursor: pointer;" onclick="return viewEditServiceAreaMaster('${serviceAreaList.area_code}','view','${serviceAreaList.status}')">View</a>&nbsp;&nbsp;&nbsp;
                               <a style="color:#2e7ea7 !important;cursor: pointer;" onclick="return viewEditServiceAreaMaster('${serviceAreaList.area_code}','edit','${serviceAreaList.status }')">Edit</a>&nbsp;&nbsp;&nbsp;
                              </td>
             ...
             --%>     
                 ...
                 <div class="row">
              	...
              	<c:if test="${index-1 == 0 && totalRecords > maxRecords}">
              		...
              		<input type="button" class='paginationButton' value="&#62;" onclick="viewServiceAreaMaster('serviceAreaMaster','${index+maxRecords-1}')">
              	</div>
              	</c:if>
              	...
              	<c:if test="${totalValue == totalRecords}">
              			<c:if test="${totalValue-maxRecords >0}">
              				<div class="form-group col-lg-2">
              		<input type="button" class='paginationButton' value="&#60;" onclick="return viewServiceAreaMaster('serviceAreaMaster','${index - maxRecords-1}');"> ${index} - ${totalValue} of ${totalRecords} 
              		<input type="button" class='paginationButton' value="&#62;" >
              		</div>
              			</c:if>
              			<c:if test="${totalValue-maxRecords <= 0}">
              	...
              	<c:if test="${index-1 != 0 &&  totalValue != totalRecords}">
              		<div class="form-group col-lg-2">
              		<input type="button" class='paginationButton' value="&#60;" onclick="return viewServiceAreaMaster('serviceAreaMaster','${index - maxRecords-1}');"> ${index} - ${index+maxRecords-1} of ${totalRecords} .
              		<input type="button" class='paginationButton' value="&#62;" onclick="viewServiceAreaMaster('serviceAreaMaster','${index+maxRecords-1}')">
              	</div> 
              	</c:if>
              	...
              	<c:if test="${(totalRecords)>0}">
              		...
              		<div class="col-lg-2" style="padding-right: 0px;">
              		  <input type="button" onclick="viewServiceAreaMaster('serviceAreaMaster',document.getElementById('pagination').value)" class="btn bg-olive btn-inline  addBtn" value="GO">
              		</div>
				</div>
              </div>   
