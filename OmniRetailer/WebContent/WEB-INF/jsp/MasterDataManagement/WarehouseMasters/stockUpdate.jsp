...
$('#searchPriceUpdate').on('input',function(e){
	debugger;
	if($(this).val().trim()==""){
		 return viewStockUpdateDetails('WarehouseStockUpdate','0');
	}
});
                      ...
                      <div class="box-body table-responsive">
                          <div class="row">
	                          <div class="form-group col-lg-12" style="text-align: center;">
		                			<label style="font-size: 22px;text-align: center"><spring:message code="stock.update.label" /></label>
		                     </div>
		                  </div>
                           ...
                           <div class="col-lg1-2 col-lg-2" style="">
                            ...
                            <div class="col-lg-1" style="    width: 12%;    background: #ccc;">
                               <div class="col-lg-12" style="    border-top: 1px solid #ccc; border-bottom: 1px solid #ccc;">
                                  <div class="row">
                                    <a onclick="viewStockUpdateDetails('WarehouseStockUpdate','0')" class="button"><spring:message code="searchbutton.label" /></a>
                                  </div>
                                 <div class="row">
                                ...
                                <br>
									    ...
									    <div class="row">
									      <div class="col-lg-1 col-xs-2" style="margin-top: 0px;">
								                <select class="form-control" id="maxRecords" style="padding: 3px 4px;" onchange="viewStockUpdateDetails('WarehouseStockUpdate','0')">
				                       ...
				                       <div class="col-lg-10 col-xs-8" style="margin-left: -15px;padding-left: 19px;margin-bottom: 10px;">
                                          <div class="wrapper">	 
                                       	...
                                       	<div class="col-lg-1" style="padding-left: 0px;margin-left:9px;">
								                <input type="button" style="" class="btn bg-olive btn-inline  addBtn" onclick="return updateStockForAll('save');" value="<spring:message code="saveall.label" />">
								          </div>
					                </div>
					       <div style="overflow: auto;">         
                          <table id="skupriceTavble"  class="table table-bordered table-striped" sty>
                               ...
                               <tr>
                             	...
                             	<%--  <th ><spring:message code="location.label" /></th> --%>
                             	 ...
                             	 <th><spring:message code="ean.label"/></th>
                             	  <th ><spring:message code="batch.number.label"/></th>
                             	 <th><spring:message code="uom.label" /></th>
                             	 <th ><spring:message code="grade.label" /></th>
                             	 <th ><spring:message code="newarrival.productrange" /></th>
                                 <th><spring:message code="color.label" /></th>
                             	 <th ><spring:message code="size.label" /></th>
                             	 <th><spring:message code="salevalue.label" />
                             	  <th ><spring:message code="quantity.in.hand.label" />
                             	 <th ><spring:message code="newquantity.in.hand.label" />
...
<%--                                  <th style="width:105px"><spring:message code="newSalePrice.label"/></th> --%>
                         ...
                         <tbody style="text-align: center;">
                          ...
                          <c:choose>
                            <c:when test="${fn:length(sku.warehouseSkuPriceLists) gt 1}">
                             <tr id="activatePurpose" >
                              <c:forEach var="pluvalue" items="${sku.warehouseSkuPriceLists}" varStatus="condition">
                                <c:set var="singleplucode" value="${pluvalue.pluCode}"  />
                                <c:set var="conditioncount" value="${condition.count}"  />
                               </c:forEach>
                            	 <td>${totalskus.count+index-1}</td>
                            	 <td style="">${sku.skuId}</td>
                                 <td><div style="width:100px">${sku.description}</div></td>
                                 <td>${sku.ean}</td>
                                 <td></td>
                                 <td>${sku.uom}</td>
                                 <td></td>
...
<!--                                  <td></td> -->
                                ...
                                <!--  <td></td> -->
                                 <td></td>
                                 <td>
                                  <c:forEach var="plu" items="${sku.warehouseSkuPriceLists}" varStatus="defaultplu">
                                    <c:if test="${defaultplu.count == 2}">
                                     <a  class="clickable" data-toggle="collapse" id="row${totalskus.count}" data-target=".row${totalskus.count}"> <span style="font-size: 17px" onclick="expandSkuPriceForStockUpdate(${totalskus.count})" id="categoryExpand${totalskus.count}"><i class="fa fa1 fa-sort-desc"></i></span></a>
                                    </c:if>
                                  </c:forEach>
                                 </td>
        				    </tr>
        				    </c:when>
        				    <c:when test="${fn:length(sku.warehouseSkuPriceLists) eq 1}">
        				      <tr id="priceListId" class="priceListId">
                               <c:forEach var="plucode" items="${sku.warehouseSkuPriceLists}" varStatus="totalpricelists">
                            	 ...
                            	 <td>${totalskus.count+index-1}</td>
                            	 <td style="" id="skuId${totalskus.count}1${totalpricelists.count}" class="skuID">${plucode.skuId}</td>
                                 <td><div style="width:115px">${plucode.description}</div></td>
                                ...
                                <%--  <fmt:formatNumber var="costPrice" type="number"  minFractionDigits="2" maxFractionDigits="2" value="${(plucode.salePrice-plucode.costPrice)}" /> --%>
                                 <td id="costPrice${totalskus.count}1${totalpricelists.count}">${plucode.costPrice}</td>
                                  ...
                                  <td id="qtyInHand${totalskus.count}1${totalpricelists.count}" class="qtyInHand"  onkeypress='return (this.innerText.length < 10)'  style="padding: 8px; background-color: #ffffff;    border: 1px solid #ccc;" contenteditable="true" >${plucode.quantityInHand} </td>
...
<%--                                  <td id="newPrice${totalskus.count}1${totalpricelists.count}" class="newPrice" onkeypress='return (this.innerText.length < 10)' onBlur="caluclateProfitability('${totalskus.count}1${totalpricelists.count}')" style="padding: 8px; background-color: #ffffff;    border: 1px solid #ccc;" contenteditable="true" >${plucode.salePrice} <input type="number" style="border: none;    border: none;height: 100%;text-align: center;"  value="${plucode.newSalePrice}"  /></td> --%>
        				    ...
        				    <tr  class="collapse rowone${totalskus.count}" id="skuPriceList${totalskus.count}1">
                                <!--    <th></th> -->
                                  <th style="padding:3px;"><div style="white-space: nowrap;padding: 5px;background: #c1c1c1;">SL No.</div></th>
                                  <th style="padding:3px;"><div style="white-space: nowrap;padding: 5px;background: #c1c1c1;"><spring:message code="report.skuid.label" /></div></th>
                                  <th style="padding:3px;"><div style="white-space: nowrap;padding: 5px;background: #c1c1c1;"><spring:message code="location.label" /></div></th>
                                  ...
                                  <th style="padding:3px;"><div style="white-space: nowrap;padding: 5px;background: #c1c1c1;"><spring:message code="salevalue.label" /></div></th>
                                       <th style="padding:3px;"><div style="white-space: nowrap;padding: 5px;background: #c1c1c1;"><spring:message code="quantity.in.hand.label" /></div></th>
                                   <th style="padding:3px;"><div style="white-space: nowrap;padding: 5px;background: #c1c1c1;"><spring:message code="newquantity.in.hand.label" /></div></th>
...
<%--                                   <th style="padding:3px;"><div style="white-space: nowrap;padding: 5px;background: #c1c1c1;"><spring:message code="newSalePrice.label" /></div></th> --%>
        				 ...
        				 <c:forEach var="plucode" items="${sku.warehouseSkuPriceLists}" varStatus="totalpricelists">
        				  ...
        				  <c:if test="${fn:length(sku.warehouseSkuPriceLists) gt 1}">
        				   <tr  class="collapse rowone${totalskus.count}" id="productrange${totalskus.count}${totalpricelists.count}" class="productRange">
        						 ...
        						 <td id="${totalpricelists.count}">${totalpricelists.count}</td>
                            	 <td  class="skuID" id="skuId${totalskus.count}1${totalpricelists.count}" style="">${plucode.skuId}</td>
...
<%--                                  <fmt:formatNumber var="newcostPrice" type="number"  minFractionDigits="2" maxFractionDigits="2" value="${((plucode.salePrice-plucode.costPrice)/plucode.costPrice)*100}" />${newcostPrice} --%>
                       ...
                       <div class="row" style="margin-right:0%;margin-left: 0.1%;">
              	          <c:if test="${index-1 == 0 && totalRecords >maxRecords}">
              		          <div class="form-group col-lg-6" style="text-align: left;">
              		                ...
              		                <input type="button" class='paginationButton' value="&#62;" onclick="viewStockUpdateDetails('WarehouseStockUpdate','${totalValue}');">
              	              </div>									
                       	</c:if>
                       	<c:if test="${totalValue == totalRecords}">
              			   <c:if test="${totalValue-maxRecords >0}">
              				<div class="form-group col-lg-6" style="text-align: left;">
              		          <input type="button" class='paginationButton' value="&#60;" onclick="return viewStockUpdateDetails('WarehouseStockUpdate','${index - (maxRecords + 1)}');"> ${index} - ${totalValue} of ${totalRecords} 
              	   ...
              	   <div class="col-lg-6" style="text-align: right;"> 
              		...
              		<div class="col-lg-2" style="padding-right: 0px;">
              		  <input type="button" onclick="viewStockUpdateDetails('WarehouseStockUpdate',document.getElementById('pagination').value)" class="btn bg-olive btn-inline  addBtn" value="GO">
              		</div>
